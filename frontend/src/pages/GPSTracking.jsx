...
import { API } from "../App";
import { MapPin, Download, Play, ArrowsClockwise, Check, Broadcast, Path, Car } from "@phosphor-icons/react";
...
export default function GPSTracking() {
  ...
  useEffect(() => {
    if (tab === "live") fetchLivePositions();
    ...
    <div className="space-y-6" data-testid="gps-page">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          ...
          <p className="text-[#52525B] mt-1">Live mapa a historie tras</p>
        </div>
      ...
      <div className="flex gap-1 bg-[#F4F4F5] p-1 rounded-lg w-fit">
        <button
          onClick={() => setTab("live")}
          className={`flex items-center gap-2 px-4 py-2 rounded-md text-sm font-medium transition-all ${tab === "live" ? "bg-white text-[#18181B] shadow-sm" : "text-[#52525B] hover:text-[#18181B]"}`}
        ...
        <button
          ...
          className={`flex items-center gap-2 px-4 py-2 rounded-md text-sm font-medium transition-all ${tab === "history" ? "bg-white text-[#18181B] shadow-sm" : "text-[#52525B] hover:text-[#18181B]"}`}
        ...
        <div className="space-y-4">
          <div className="flex items-center gap-3">
            ...
            <Button
              ...
              className={autoRefresh ? "bg-[#16A34A] hover:bg-[#15803D]" : ""}
          ...
          <div className="grid grid-cols-1 lg:grid-cols-4 gap-4">
            ...
            <div className="space-y-2 max-h-[600px] overflow-y-auto">
              {livePositions.map(pos => (
                <div
                  key={pos.vehicle_id}
                  className="bg-white border border-[#E4E4E7] rounded-md p-3"
                  data-testid={`live-vehicle-${pos.vehicle_id}`}
                >
                ...
                <MapContainer center={[50.0755, 14.4378]} zoom={12} style={{ height: "100%", width: "100%" }} scrollWheelZoom={true}>
                  ...
                  {validLivePositions.map(pos => (
                    <Marker
                      ...
                      icon={pos.ignition ? VEHICLE_ICON_ACTIVE : VEHICLE_ICON_INACTIVE}
                    >
                      <Popup>
                        <div className="text-sm">
                          <p className="font-semibold">{pos.vehicle_info}</p>
                          <p>{pos.speed} km/h {pos.ignition ? "- v pohybu" : "- stojí"}</p>
                          <p className="text-xs text-gray-500">{pos.timestamp ? new Date(pos.timestamp).toLocaleString("cs-CZ") : ""}</p>
                        </div>
                      </Popup>
                    </Marker>
                  ))}
